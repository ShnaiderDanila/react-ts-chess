...
class Cell {
  ...
  // Методы для передвижения различных фигур
  isEmptyVertical(target: Cell): boolean {
    ...
    for (let y = min + 1; y < max; y++) {
      if (this.board.getCell(this.x, y).figure) {
        return false;
      }
    }
  ...
  isPawnAttack(target: Cell): boolean {
    ...
    return target.y === this.y + direction && (target.x === this.x + 1 || target.x === this.x - 1);
  }

  isKnightAttack(target: Cell): boolean {
  ...
  public moveFigure(targetCell: Cell) {
    if (this.figure && this.figure?.canMove(targetCell)) {
      ...
      // Если съели фигуру, добавили её в счет
      if (targetCell.figure) {
        this.board.addLostFigure(targetCell.figure);
      }
